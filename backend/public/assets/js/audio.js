...
export function toggleSound(enabled) {
  ...
  try { localStorage.setItem('kaizen_sound', JSON.stringify(soundEnabled)); } catch {}
  return soundEnabled;
}
...
export function loadSoundPreference() {
  try {
    const saved = localStorage.getItem('kaizen_sound');
...
export function playTactileClick(freq = 800, duration = 0.03) {
