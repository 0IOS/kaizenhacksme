...
$pageTitle = 'Register — Kaizen Hacks';
$pageDescription = 'Create a Kaizen Hacks account to join events, register for ideathons, and start building.';
$baseUrl = '';
...
<main class="min-h-screen flex items-center justify-center px-5 sm:px-8 pt-28 pb-20">
    <div class="relative w-full max-w-md">
            ...
            <div class="px-8 py-10 sm:px-10 sm:py-12">
                ...
                <div class="mb-8">
                    <div class="mono text-[9px] tracking-[0.28em] uppercase text-slate-500 dark:text-[#565C57] mb-2">
                        KAIZEN_SYSTEM // NEW_IDENTITY
                    </div>
                    <h1 class="font-display text-3xl font-bold tracking-tight text-slate-950 dark:text-[#F5F5F0]">
                        REGISTER <span class="text-emerald-700 dark:text-accent">//</span> KAIZEN_SYSTEM
                    </h1>
                </div>

                <!-- Flash Messages -->
                <?php if ($flashError): ?>
                    <div id="signup-feedback" class="mb-6 px-4 py-3 text-xs mono border border-red-300 dark:border-red-500/30 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400">
                        <?= htmlspecialchars($flashError, ENT_QUOTES, 'UTF-8') ?>
                    </div>
                <?php elseif ($flashSuccess): ?>
                    <div id="signup-feedback" class="mb-6 px-4 py-3 text-xs mono border border-emerald-300 dark:border-accent/30 bg-emerald-50 dark:bg-accent/10 text-emerald-700 dark:text-accent">
                        <?= htmlspecialchars($flashSuccess, ENT_QUOTES, 'UTF-8') ?>
                    </div>
                <?php else: ?>
                    <div id="signup-feedback" class="mb-6 px-4 py-3 text-xs mono border hidden"></div>
                <?php endif; ?>
                ...
                <form id="signup-form" class="space-y-5">
                    ...
                    <!-- Name -->
                    <div>
                        <label for="signup-name" class="block mono text-[10px] tracking-[0.2em] uppercase text-slate-600 dark:text-[#565C57] mb-2 font-bold">
                            FULL_NAME
                        </label>
                        <input
                            type="text"
                    ...
                    <!-- Email -->
                    <div>
                        <label for="signup-email" class="block mono text-[10px] tracking-[0.2em] uppercase text-slate-600 dark:text-[#565C57] mb-2 font-bold">
                            EMAIL_ADDRESS
                        </label>
                        <input
                            type="email"
                    ...
                    <!-- Password -->
                    <div>
                        <label for="signup-password" class="block mono text-[10px] tracking-[0.2em] uppercase text-slate-600 dark:text-[#565C57] mb-2 font-bold">
                            PASSWORD
                        </label>
                        <input
                            type="password"
                    ...
                    <!-- Confirm Password -->
                    <div>
                        <label for="signup-password-confirm" class="block mono text-[10px] tracking-[0.2em] uppercase text-slate-600 dark:text-[#565C57] mb-2 font-bold">
                            CONFIRM_PASSWORD
                        </label>
                        <input
                            type="password"
                ...
                <div class="mt-8 pt-6 border-t border-slate-200 dark:border-[#1A1C1A] text-center">
                    <p class="mono text-xs text-slate-600 dark:text-[#565C57]">
                        ALREADY REGISTERED?
                        <a href="/login.php" class="text-emerald-700 dark:text-accent font-bold hover:underline">
                            LOGIN →
                        </a>
                    </p>
                </div>
        ...
        <div class="mt-4 flex items-center justify-center gap-2 mono text-[9px] tracking-[0.22em] uppercase text-slate-500 dark:text-[#565C57]">
            ...
            <span>IDENTITY_MODULE v1.0 // SECURE_CHANNEL</span>
        </div>
    </div>
</main>
