...
export const MarqueeTicker: React.FC = () => {
  ...
  const smoothVelocity = useSpring(velocity, { stiffness: 140, damping: 32, mass: 0.5 });
  const skewX = useTransform(smoothVelocity, [-3000, 3000], [-5, 5], { clamp: true });
  const shiftY = useTransform(smoothVelocity, [-3000, 3000], [2, -2], { clamp: true });
  ...
  return (
    <div className="relative w-full border-y border-slate-300 dark:border-[#1A1C1A] bg-slate-200/70 dark:bg-[#080A09] py-3.5 overflow-hidden select-none">
      ...
      <motion.div style={reduced ? undefined : { skewX, y: shiftY }} className="will-change-transform">
