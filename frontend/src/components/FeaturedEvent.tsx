...
import { ScrubIn, CornerBrackets, useCursorParallax } from '../lib/motion';

interface FeaturedEventProps {
  onSelectEventDetail: () => void;
}

const NUM_DIRECTIONS = ['left', 'up', 'right', 'diag-right'] as const;
...
export const FeaturedEvent: React.FC<FeaturedEventProps> = ({ onSelectEventDetail }) => {
  ...
  /* Gentle cursor depth on the centerpiece title */
  const { x: cx, y: cy } = useCursorParallax(1);
  const titleX = useTransform(cx, (v) => v * 4);
  const titleY = useTransform(cy, (v) => v * 3);

  return (
    <section
      ref={containerRef}
      id="featured-event"
      className="py-16 sm:py-24 px-5 sm:px-8 lg:px-12 shell select-none overflow-hidden"
    >
      {/* Section Header */}
      <div className="flex flex-wrap items-end justify-between gap-4 mb-8 border-b border-slate-300 dark:border-[#1A1C1A] pb-4">
        <ScrubIn from="left" distance={64} rotate={-0.5}>
          <div>
        ...
        {/* Status Pill */}
        <ScrubIn from="right" distance={48} lag={0.08}>
      ...
      {/* Main Visual Panel */}
      <ScrubIn from="none" distance={0} scaleFrom={0.985} className="relative group event-panel border border-slate-300 dark:border-[#1A1C1A] bg-white dark:bg-[#0B0D0C] hover:border-emerald-600/60 dark:hover:border-accent/50 transition-all duration-300 overflow-hidden shadow-md hover:shadow-2xl">
        <CornerBrackets />
        {/* Hover glow halo */}
        ...
        <div className="relative z-10 p-6 sm:p-10 lg:p-14 flex flex-col justify-between min-h-[580px] lg:min-h-[640px]">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-slate-200 dark:border-[#1A1C1A] pb-6">
            <div className="flex flex-wrap items-center gap-3 sm:gap-6 text-xs mono text-slate-700 dark:text-[#A9ADA9]">
              <ScrubIn from="down" distance={22} lag={0.05}>
              ...
              <ScrubIn from="down" distance={22} lag={0.11}>
              ...
              <ScrubIn from="down" distance={22} lag={0.17}>
                <a
              ...
              <ScrubIn from="down" distance={22} lag={0.23}>
            ...
            {/* Status HUD */}
            <ScrubIn from="right" distance={36} lag={0.12}>
          ...
          <div className="my-8 lg:my-10">
            ...
            <ScrubIn from="none" distance={0} scaleFrom={0.94}>
            ...
            <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6 border-y border-slate-200 dark:border-[#1A1C1A] py-6 max-w-4xl">
              <ScrubIn from={NUM_DIRECTIONS[0]} distance={44} lag={0.04}>
                <div>
              ...
              <ScrubIn from={NUM_DIRECTIONS[1]} distance={44} lag={0.1}>
                <div>
              ...
              <ScrubIn from={NUM_DIRECTIONS[2]} distance={44} lag={0.16}>
                <div>
              ...
              <ScrubIn from={NUM_DIRECTIONS[3]} distance={44} lag={0.22}>
                <div>
          ...
          <div className="pt-6 border-t border-slate-200 dark:border-[#1A1C1A] flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-5 py-3.5 bg-transparent hover:bg-slate-100 dark:hover:bg-[#1A1C1A] border border-slate-300 dark:border-[#1A1C1A] text-slate-900 dark:text-[#F5F5F0] text-xs mono uppercase tracking-wider transition-colors cursor-pointer active:scale-[0.98]"
              >
                VIEW FULL SPEC
              </button>
              ...
              <a
                ...
                className="group px-7 py-3.5 bg-emerald-600 hover:bg-emerald-700 dark:bg-accent dark:hover:opacity-90 text-white dark:text-[#050605] mono font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-3 transition-all duration-200 cursor-pointer shadow-sm active:scale-[0.98]"
              >
